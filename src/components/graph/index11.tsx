    ...
    const initForceSimulation = () => {
        ...
        nodes
            ...
            .attr(
                'transform',
                ({ level }) =>
                    `translate(${-svgData.rediusList[level]}, ${-svgData.rediusList[level]})`
            )
            .append('xhtml:div')
            .attr('class', 'text-wrapper')
            .append('span')
            .attr('style', ({ level }) => {
                return `display:inline-block;height:${
                    2 * svgData.rediusList[level]
                }px;line-height:${2 * svgData.rediusList[level]}px`;
            })
            .text((data) => {
                return data.name;
            });
        ...
        nodes
            ...
            .attr(
                'transform',
                ({ level }) =>
                    `translate(${-1.5 * svgData.rediusList[level]}, ${
                        -1.5 * svgData.rediusList[level]
                    })`
            )
            .insert('xhtml:div')
            .attr('style', ({ colorIdx }) => {
