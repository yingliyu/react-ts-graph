...
const Graph: React.FC<IGraphProps> = (props: IGraphProps) => {
  ...
  const initForceSimulation = () => {
    ...
    // const nodeCollide = d3
    //   .forceCollide(data => {
    //     const d = data as INode;
    //     if (d.name === '吴恩达' && d.level === 1) {
    //       d.fx = svgData.svgW / 2; // 设置特定节点固定x坐标
    //       d.fy = svgData.svgH / 2;
    //     }
    //     return 120 - d.level * 18;
    //   })
    //   .iterations(0.5)
    //   .strength(0.5);
    ...
    simulation = d3
      ...
      .force(
        'collision',
        d3.forceCollide().radius((d: INode) => {
          console.log(d);
          return svgData.rediusList[d.level] + d.level;
        }),
      )
      .force('charge', d3.forceManyBody<INode>().strength(-300));
